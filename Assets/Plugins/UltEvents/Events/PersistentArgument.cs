...
namespace UltEvents
{
    ...
    public sealed class PersistentArgument
    {
        ...
        [SerializeField] internal PersistentArgumentType _Type;

        [SerializeField] internal int _Int;

        [SerializeField] internal string _String;

        [SerializeField] internal float _X, _Y, _Z, _W;

        [SerializeField] internal Object _Object;

        /************************************************************************************************************************/

        [NonSerialized] private Type _SystemType;

        [NonSerialized] internal bool _HasSystemType;
        ...
        {
            if (_Type != type)
                throw new InvalidOperationException(Names.PersistentArgument.Full.Type + " is " + _Type +
                                                    " but should be " + type);
        }
        ...
        ///     <para></para>
        ///     If the 'type' can be inherited (such as an Enum or Object), the 'assemblyQualifiedName' will be assigned as well
        ///     (otherwise null).
        /// </summary>
        public static PersistentArgumentType GetArgumentType(Type type, out string assemblyQualifiedName,
            out int linkIndex)
        {
            linkIndex = 0;
...
#pragma warning restore IDE0017 // Simplify object initialization
        ...
        public override string ToString()
        {
            switch (_Type)
            {
                ...
                case PersistentArgumentType.Parameter:
                    return Names.PersistentArgument.Class + ": SystemType=" + SystemType + ", Value=Parameter" +
                           ParameterIndex;
                case PersistentArgumentType.ReturnValue:
                    return Names.PersistentArgument.Class + ": SystemType=" + SystemType + ", Value=ReturnValue" +
                           ReturnedValueIndex;
                default:
