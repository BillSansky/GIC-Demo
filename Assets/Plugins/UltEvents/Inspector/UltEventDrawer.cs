...
namespace UltEvents.Editor
{
    ...
    internal sealed class UltEventDrawer : PropertyDrawer
    {
        ...
        private float CalculateCallHeight(int index)
        {
            ...
            {
                var height =
                    EditorGUI.GetPropertyHeight(_CurrentCallList.serializedProperty.GetArrayElementAtIndex(index));
        ...
        private void CachePersistentCallList(SerializedProperty eventProperty)
        {
            ...
            if (!PropertyPathToList.TryGetValue(path, out _CurrentCallList))
            {
                ...
                _CurrentCallList =
                    new ReorderableList(eventProperty.serializedObject, eventProperty, true, true, true, true)
                    {
                    ...
                    DrawerState.Current.EventProperty.serializedObject.targetObjects.Length == 1)
                {
                    if (DrawerState.Current.Event.HasAnyDynamicCalls())
                        _CurrentCallList.footerHeight +=
                            DrawerState.Current.Event.GetDynamicCallInvocationListCount() *
        ...
        public void DoFooterGUI(Rect area)
        {
            ...
            else if (DrawerState.Current.Event != null)
            {
                ...
                {
                    var isExpanded = EditorGUI.Foldout(area, _CurrentCallList.serializedProperty.isExpanded,
                    ...
                    {
                        DoDynamicListenerGUI(area.x, area.y + EditorGUIUtility.singleLineHeight - 1, width,
                            DrawerState.Current.Event);
                    }
                }
            ...
            if (GUI.Button(area, ReorderableList.defaultBehaviours.iconToolbarPlus,
            ...
            using (new EditorGUI.DisabledScope(
                _CurrentCallList.index < 0 || _CurrentCallList.index >= _CurrentCallCount))
            {
                if (GUI.Button(area, ReorderableList.defaultBehaviours.iconToolbarMinus,
                    ReorderableList.defaultBehaviours.preButton))
                {
        ...
        public static void DoDelegateGUI(Rect area, Delegate del)
        {
            ...
            else if (del.Method.DeclaringType.IsDefined(
                typeof(System.Runtime.CompilerServices.CompilerGeneratedAttribute), true))
            {
