...
namespace UltEvents.Editor
{
    ...
    internal sealed class PersistentArgumentDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect area, SerializedProperty argumentProperty, GUIContent label)
        {
            ...
            switch ((PersistentArgumentType) typeProperty.enumValueIndex)
            {
                case PersistentArgumentType.None:
                    DoErrorMessageGUI(area, argumentProperty, label, "Error: argument type not set");
                    break;

                case PersistentArgumentType.Bool:
                    DoBoolGUI(area, argumentProperty, label);
                    break;
                case PersistentArgumentType.String:
                    DoStringGUI(area, argumentProperty, label);
                    break;
                case PersistentArgumentType.Int:
                    DoIntGUI(area, argumentProperty, label);
                    break;
                case PersistentArgumentType.Enum:
                    DoEnumGUI(area, argumentProperty, label);
                    break;
                case PersistentArgumentType.Float:
                    DoFloatGUI(area, argumentProperty, label);
                    break;
                case PersistentArgumentType.Vector2:
                    DoVector2GUI(area, argumentProperty, label);
                    break;
                case PersistentArgumentType.Vector3:
                    DoVector3GUI(area, argumentProperty, label);
                    break;
                case PersistentArgumentType.Vector4:
                    DoVector4GUI(area, argumentProperty, label);
                    break;
                case PersistentArgumentType.Quaternion:
                    DoQuaternionGUI(area, argumentProperty, label);
                    break;
                case PersistentArgumentType.Color:
                    DoColorGUI(area, argumentProperty, label);
                    break;
                case PersistentArgumentType.Color32:
                    DoColor32GUI(area, argumentProperty, label);
                    break;
                case PersistentArgumentType.Rect:
                    DoRectGUI(area, argumentProperty, label);
                    break;
                case PersistentArgumentType.Object:
                    DoObjectGUI(area, argumentProperty, label);
                    break;

                case PersistentArgumentType.Parameter:
                case PersistentArgumentType.ReturnValue:
                    DoLinkedValueGUI(area, argumentProperty, label);
                    break;
        ...
        private static void DoErrorMessageGUI(Rect area, SerializedProperty argumentProperty, GUIContent label,
            string message)
        {
        ...
        private static void DoColor32GUI(Rect area, SerializedProperty argumentProperty, GUIContent label)
        {
            ...
            var value = EditorGUI.ColorField(area, label,
                new Color32((byte) (intValue), (byte) (intValue >> 8), (byte) (intValue >> 16),
                    (byte) (intValue >> 24)));
        ...
        private static void DoObjectGUI(Rect area, SerializedProperty argumentProperty, GUIContent label)
        {
            ...
            var type = SerializedPropertyAccessor.GetValue<PersistentArgument>(argumentProperty).SystemType ??
                    ...
                    case PersistentArgumentType.ReturnValue:
                        ...
                        else
                        {
                            ...
                            if (DrawerState.Current.callIndex >= 0 && DrawerState.Current.callIndex <= callIndex)
                            {
                                GUI.color = PersistentCallDrawer.ErrorFieldColor;
                                label.tooltip =
                                    "The linked method must be called before this argument can retrieve its return value";
                            }
        ...
        private static void ShowLinkMenu(Rect area, SerializedProperty argumentProperty, Type systemType, int linkIndex,
            PersistentArgumentType linkType)
        {
            var typeProperty = argumentProperty.FindPropertyRelative(Names.PersistentArgument.Type);
            ...
            for (int i = 0; i < DrawerState.Current.PreviousCallCount; i++)
            {
                ...
                var content = new GUIContent(string.Concat("Returned Value ", i.ToString(), " (",
                    MethodSelectionMenu.GetMethodSignature(method, true), ")"));
        ...
        private static void DoLinkModeToggleGUI(Rect area, SerializedProperty argumentProperty, int linkIndex,
            PersistentArgumentType linkType)
        {
            if (linkIndex < 0)
                return;
                ...
                SerializedPropertyAccessor.ModifyValues<PersistentArgument>(argumentProperty, (argument) =>
                {
                    ...
                    else
                    {
                        ...
                        switch (argumentType)
                        {
                            ...
                            case PersistentArgumentType.Enum:
                            case PersistentArgumentType.Object:
                                argument._String = DrawerState.Current.CurrentParameter.ParameterType
        ...
        ///     <para></para>
        ///     This method works properly for some enum configurations not supported by EditorGUI.EnumMaskField or
