    ...
    internal sealed class PersistentCallDrawer : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect area, SerializedProperty callProperty, GUIContent label)
        {
            ...
            label = EditorGUI.BeginProperty(propertyarea, label, callProperty);
            {
                ...
                EditorGUI.showMixedValue = DrawerState.Current.PersistentArgumentsProperty.hasMultipleDifferentValues ||
                ...
                if (method != null)
                {
                    ...
                    if (DrawerState.Current.callParameters.Length ==
                        DrawerState.Current.PersistentArgumentsProperty.arraySize)
                    {
                        ...
                        for (int i = 0; i < DrawerState.Current.callParameters.Length; i++)
                        {
                            ...
                            var argumentProperty =
                                DrawerState.Current.PersistentArgumentsProperty.GetArrayElementAtIndex(i);
                    ...
                    {
                        Debug.LogError(
                            "Method parameter count doesn't match serialized argument count " + DrawerState.Current
                                                                                                  .callParameters.Length
                                                                                              + " : " + DrawerState
                                                                                                  .Current
        ...
        private static void DoMethodFieldGUI(Rect area, MethodBase method, bool autoOpenMethodMenu)
        {
            EditorGUI.BeginProperty(area, null, DrawerState.Current.MethodNameProperty);
            {
                ...
                PopupLabelStyle.fontStyle = DrawerState.Current.MethodNameProperty.prefabOverride
                    ? FontStyle.Bold
                    : FontStyle.Normal;
        ...
        private static void DoGetSetToggleGUI(ref Rect area, MethodBase method)
        {
            ...
            if (isGet || isSet)
            {
                ...
                var oppositePropertyMethod =
                    method.DeclaringType.GetMethod(methodName, UltEventUtils.AnyAccessBindings);
        ...
        private static void DoTargetFieldGUI(Rect area, SerializedProperty targetProperty,
            SerializedProperty methodNameProperty, out bool autoOpenMethodMenu)
        {
        ...
        private static void DoTargetObjectFieldGUI(Rect area, SerializedProperty targetProperty,
            ref bool autoOpenMethodMenu)
        {
            ...
            if (_AllTypes == null)
            {
                ...
                for (int i = 0; i < assemblies.Length; i++)
                {
                    ...
                    for (int j = 0; j < types.Length; j++)
                    {
                        var type = types[j];
                        if (
                            !type.ContainsGenericParameters && // No Generics (because the type picker field doesn't let you pick generic parameters).
                            ...
                            type.GetMethods(UltEventUtils.StaticBindings).Length > 0
                        ) // No types without any static methods.
                        {
        ...
        private static void HandleTargetFieldDragAndDrop(Rect area, ref bool autoOpenMethodMenu)
        {
            ...
            switch (Event.current.type)
            {
                ...
                case EventType.DragPerform:
                {
                    ...
                    if (dragging != null && dragging.Length == 1)
                    {
                        SetBestTarget(DrawerState.Current.TargetProperty.objectReferenceValue, dragging[0],
                            out autoOpenMethodMenu);
        ...
        private static void SetBestTarget(Object oldTarget, Object newTarget, out bool autoOpenMethodMenu)
        {
            ...
            autoOpenMethodMenu = BoolPref.AutoOpenMenu && newTarget != null &&
                                 DrawerState.Current.call.GetMethodSafe() == null;
        }
        ...
        public static bool DoRemoveButtonGUI(Rect rowArea)
        {
            ...
            return GUI.Button(rowArea, ReorderableList.defaultBehaviours.iconToolbarMinus,
