...
#endif
{
    ...
    public class SerializedPropertyAccessor
    {
        ...
        public static void SetValue(SerializedProperty property, object targetObject, object value)
        {
            switch (property.propertyType)
            {
                ...
                case SerializedPropertyType.FixedBufferSize:
                    throw new InvalidOperationException("SetValue failed: " + Names.SerializedPropertyFixedBufferSize +
                                                        " is read-only.");
        ...
        /// <summary>
        ///     Gets a <see cref="SerializedPropertyAccessor" /> that can be used to access the details of the specified
        ///     'property'.
        /// </summary>
        ...
        private static SerializedPropertyAccessor GetAccessor(string propertyPath, ref Type propertyType)
        {
            ...
            if (!pathToAccessor.TryGetValue(propertyPath, out accessor))
            {
                ...
                // Array.
                if (nameStartIndex > 6 && nameStartIndex < propertyPath.Length - 7 &&
                    string.Compare(propertyPath, nameStartIndex - 6, ".Array.data[", 0, 12) == 0)
                {
        ...
        /// <summary>
        ///     Gets the value of the from the <see cref="Parent" /> (if there is one), then uses it to get the value from the
        ///     <see cref="Field" />.
        /// </summary>
        ...
        /// <summary>
        ///     Gets the value of the from the <see cref="Parent" /> (if there is one), then uses it to set the value from the
        ///     <see cref="Field" />.
        /// </summary>
    ...
    internal class SerializedPropertyArrayAccessor : SerializedPropertyAccessor
    {
        ...
        private static Type GetElementType(Type fieldType)
        {
            ...
            {
                Debug.LogWarning(Names.SerializedPropertyArrayAccessor + ": unable to determine element type for " +
                                 fieldType);
                return fieldType;
            }
        }
