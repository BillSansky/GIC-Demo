...
namespace UltEvents.Editor
{
    ...
    public static class ObjectPicker
    {
        ...
        public static T Draw<T>(Rect area, T selected, Func<List<T>> getOptions, int suggestions,
            Func<T, GUIContent> getLabel, Func<T> getDragAndDrop,
            GUIStyle style)
        {
        ...
        public static T DrawLayout<T>(T selected, Func<List<T>> getOptions, int suggestions,
            Func<T, GUIContent> getLabel, Func<T> getDragAndDrop,
        ...
        {
            return DrawLayout(selected, getOptions, suggestions, getLabel, getDragAndDrop, InternalGUI.TypeButtonStyle,
                layoutOptions);
        }
        ...
        public static T DrawEditorLayout<T>(GUIContent label, T selected, Func<List<T>> getOptions, int suggestions,
            Func<T, GUIContent> getLabel, Func<T> getDragAndDrop, GUIStyle style,
            params GUILayoutOption[] layoutOptions)
        {
            GUILayout.BeginHorizontal();
            {
                GUILayout.Label(label, GUILayout.Width(EditorGUIUtility.labelWidth - 4));
                selected = DrawLayout(selected, getOptions, suggestions, getLabel, getDragAndDrop, style,
                    layoutOptions);
            }
        ...
        {
            return DrawEditorLayout(label, selected, getOptions, suggestions, getLabel, getDragAndDrop,
                InternalGUI.TypeButtonStyle, options);
        }
        ...
        public static Type DrawTypeField(Rect area, Type selected, Func<List<Type>> getOptions, int suggestions,
            GUIStyle style)
        {
        ...
        public static Type DrawAssetTypeField(Rect area, Type selected, Func<List<Type>> getOptions, int suggestions,
            GUIStyle style)
        {
            return Draw(area, selected, getOptions, suggestions,
                getLabel: (type) => new GUIContent(type != null ? type.GetNameCS() : "null",
        ...
        /// <summary>
        ///     Draws a field which lets you pick a <see cref="Type"></see> from a list and returns the selected
        ///     <see cref="Type.AssemblyQualifiedName" />.
        /// </summary>
        public static string DrawTypeField(Rect area, string selectedTypeName, Func<List<Type>> getOptions,
            int suggestions, GUIStyle style)
        {
        ...
        private static void CheckDragAndDrop<T>(Rect area, ref T selected, Func<List<T>> getOptions,
            Func<T> getDragAndDrop)
        {
    ...
    internal sealed class ObjectPickerWindow : EditorWindow
    {
        ...
        [NonSerialized] private int _LabelWidthCalculationProgress;

        [NonSerialized] private float _MaxLabelWidth;

        [NonSerialized] private IList _Objects;

        [NonSerialized] private Vector2 _ScrollPosition;

        [NonSerialized] private string _SearchText = "";

        [NonSerialized] private object _SelectedObject;

        [NonSerialized] private int _Suggestions;
        ...
        public static void Show<T>(int fieldID, T selected, List<T> objects, int suggestions,
        ...
        private void DrawOption(Rect area, List<GUIContent> labels, IList objects, int index)
        {
            ...
            var isOn = GUI.Toggle(area, wasOn, labels[index],
                wasOn ? InternalGUI.SelectedButtonStyle : InternalGUI.ButtonStyle);
            if (isOn != wasOn)
            {
                if (wasOn)
                {
